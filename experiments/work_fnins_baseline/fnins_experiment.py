...
def make_rr_amp_series(signal: np.ndarray, rpeaks: np.ndarray, fs: int):
    """Return (times, rr_repaired, amp, rr_raw). rr_raw is the un-repaired RR for SQI gating."""
    ...
    rr_raw = np.diff(times).astype(np.float32)
    rr = repair_rr_intervals(rr_raw)
    amp = signal[rpeaks[1:]].astype(np.float32)
    return times[1:].astype(np.float32), rr.astype(np.float32), amp.astype(np.float32), rr_raw.astype(np.float32)


def window_bad_rr_fraction(times: np.ndarray, rr_raw: np.ndarray, start_sec: float, duration_sec: float) -> float:
    """Fraction of beats in the window whose RAW RR is physiologically implausible.

    Flags RR outside [0.33, 2.0] s, or sudden jumps > 50% of the window median RR.
    Used as a signal-quality gate: high fraction means unreliable R-peaks in that minute.
    """
    mask = (times >= start_sec) & (times < start_sec + duration_sec)
    rr = rr_raw[mask]
    if len(rr) < 3:
        return 1.0
...
def feature_cache_path(args, dataset: str, record_id: str) -> Path:
    ...
    if float(getattr(args, "max_bad_rr_fraction", 0.0)) > 0.0:
        suffix += f"_sqi{str(args.max_bad_rr_fraction).replace('.', 'p')}"
    if dataset == "ucddb":
...
def build_feature_record(args, dataset: str, record_id: str) -> FeatureRecord:
    ...
    times, rr, amp, rr_raw = make_rr_amp_series(signal, rpeaks, fs)
    max_bad_rr = float(getattr(args, "max_bad_rr_fraction", 0.0))  # P3 signal-quality gate (0 = off)
    ...
    for center in centers_iter:
        ...
        if feat is None:
            skipped += 1
            continue
        if max_bad_rr > 0.0 and window_bad_rr_fraction(times, rr_raw, start, duration_sec) > max_bad_rr:
    ...
    stats = {
        ...
        "skipped_low_quality": int(skipped_low_quality),
        "max_bad_rr_fraction": float(max_bad_rr),
...
class LearnableTemporalEncoding(nn.Module):
    def __init__(self, d_model: int, max_len: int = 512, dropout: float = 0.0):
        super().__init__()
        if d_model < 4:
            raise ValueError("d_model must be at least 4 for temporal encoding")
        ...
        self.distance_weight = nn.Parameter(torch.randn(1) * 0.02)
        self.distance_bias = nn.Parameter(torch.zeros(1))
        self.periodic_weight = nn.Parameter(torch.randn(periodic_dim) * 0.02)
        self.periodic_bias = nn.Parameter(torch.zeros(periodic_dim))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        seq_len = x.size(1)
        if seq_len > self.max_len:
            raise ValueError(f"Sequence length {seq_len} exceeds max temporal length {self.max_len}")

        t = torch.linspace(-1.0, 1.0, seq_len, device=x.device, dtype=x.dtype).unsqueeze(1)
        linear = t * self.linear_weight.to(dtype=x.dtype) + self.linear_bias.to(dtype=x.dtype)
        distance = t.abs() * self.distance_weight.to(dtype=x.dtype) + self.distance_bias.to(dtype=x.dtype)
        periodic = torch.sin(t * self.periodic_weight.to(dtype=x.dtype) + self.periodic_bias.to(dtype=x.dtype))
        temporal = torch.cat([linear, distance, periodic], dim=1).unsqueeze(0)
        return self.dropout(x + temporal)
...
class CNNTransformer(nn.Module):
    def __init__(
        ...
        self.temporal_encoder = LearnableTemporalEncoding(d_model=d_model, max_len=512, dropout=dropout)
...
def load_pretrained(model: nn.Module, path: Path, device: torch.device) -> None:
    ...
    try:
        ...
        allowed_missing = {
            "pos_encoder.pe",
            "temporal_encoder.linear_weight",
            "temporal_encoder.linear_bias",
            "temporal_encoder.distance_weight",
            "temporal_encoder.distance_bias",
            "temporal_encoder.periodic_weight",
            "temporal_encoder.periodic_bias",
        }
        allowed_unexpected = {"pos_encoder.pe"}
        if missing.issubset(allowed_missing) and unexpected.issubset(allowed_unexpected):
            print("Loaded checkpoint with newly initialized temporal encoding parameters.")
            return
        raise exc
...
def build_criterion(args, y_train, device):
    """Build the loss. --loss ce | wce (class-weighted) | focal (weighted focal)."""
    counts = np.bincount(np.asarray(y_train, dtype=np.int64), minlength=2)
    weight = torch.tensor(len(y_train) / (2.0 * np.maximum(counts, 1)), dtype=torch.float32, device=device)
    if args.loss == "ce":
        print("Loss: plain cross entropy")
        return nn.CrossEntropyLoss()
    if args.loss == "wce":
        print(f"Loss: class-weighted cross entropy weights={weight.tolist()}")
        return nn.CrossEntropyLoss(weight=weight)
    if args.loss == "focal":
        print(f"Loss: weighted focal gamma={args.focal_gamma} weights={weight.tolist()}")
        return SoftmaxFocalLoss(gamma=args.focal_gamma, weight=weight)
...
def run_training(args) -> dict:
    ...
    criterion = build_criterion(args, split.y_train, device)
...
def main() -> None:
    ...
    parser.add_argument("--loss", choices=["ce", "wce", "focal"], default="ce")
