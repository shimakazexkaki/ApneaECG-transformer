...
def load_raw_models(model_paths, device):
    models = []
...
def predict_ensemble_scores(models, x, y, batch_size, device):
    all_scores = []
    labels = None
    for model in models:
        labels, scores = mixed_trainer.predict_scores(model, x, y, batch_size, device)
        all_scores.append(scores)
    return labels, np.mean(np.stack(all_scores, axis=0), axis=0).astype(np.float32)
...
def collect_scores(models, domain, records, args, device):
    labels_all = []
    raw_all = []
    smooth_all = []
    for record_id in records:
        ...
        for channel in channel_list:
            ...
            if len(y) == 0:
                continue
            labels, scores = predict_ensemble_scores(models, x, y, args.batch_size, device)
...
def run(args):
    ...
    models = load_raw_models(args.models, device)
    ...
    y_u_val, u_val_raw, u_val_smooth = collect_scores(models, "ucddb", ucddb_val_records, args, device)
    y_a_val, a_val_raw, a_val_smooth = collect_scores(models, "apnea", apnea_val_records, args, device)
    y_u_test, u_test_raw, u_test_smooth = collect_scores(models, "ucddb", ucddb_test_records, args, device)
    y_a_test, a_test_raw, a_test_smooth = collect_scores(models, "apnea", apnea_test_records, args, device)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate raw ECG model ensembles with temporal smoothing.")
    ...
    parser.add_argument("--result-name", default="ensemble_sequence_smoothing_results.json")
